    ...
    public function manejarRequest($method) {
        switch ($method) {
            ...
            case 'POST':
                ...
                $idDispositivo = $this->modelo->crear($data); // ✅ Solo una vez
            ...
            case 'DELETE':
    file_put_contents("log.txt", "?? Entr� al DELETE\n", FILE_APPEND);

    $input = file_get_contents("php://input");
    file_put_contents("log.txt", "?? Body recibido: $input\n", FILE_APPEND);

    if (empty($input)) {
        file_put_contents("log.txt", "? JSON vac�o\n", FILE_APPEND);
        echo json_encode(["success" => false, "message" => "JSON vac�o"]);
        return;
    }
